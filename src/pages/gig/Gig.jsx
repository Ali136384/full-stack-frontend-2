import React from "react";
import "./Gid.scss";
import {
  faClock,
  faHourglassHalf,
  faStar,
  faThumbsDown,
  faThumbsUp,
} from "@fortawesome/free-regular-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faCheck,
  faEarthAmericas,
  faSpinner,
} from "@fortawesome/free-solid-svg-icons";
import Slide from "../../components/slide/Slide";
import { Slider } from "infinite-react-carousel";
function Gig() {
  return (
    <>
      <div className="gig">
        <div className="container">
          <div className="left">
            <p className=" font-bold mb-[30px]  text-[29px]">
              I Will Create A Responsive Website For You
            </p>
            <div className="profile-user mb-[10px]">
              <span>
                <img
                  className="profile-pic"
                  src="data:image/png;base64,"
                  alt=""
                />
              </span>
              <p>Ali Haseni</p>
              <p>
                {" "}
                <FontAwesomeIcon icon={faStar} />
                <FontAwesomeIcon icon={faStar} />
                <FontAwesomeIcon icon={faStar} />
                <FontAwesomeIcon icon={faStar} />
                <FontAwesomeIcon icon={faStar} />
              </p>
            </div>
            <Slider arrowsScroll={1} slidesToShow={1} dots>
              <img
                className="slider-img"
                src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                alt=""
              />{" "}
              <img
                className="slider-img"
                src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs/105848244/original/ea26f6f6febd4c7f416631f44334f7358c812960/create-a-premium-and-modern-wordpress-website.jpg"
                alt=""
              />{" "}
              <img
                className="slider-img"
                src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                alt=""
              />
            </Slider>
            <div className="about-gig">
              <h2>About The Service</h2>
              <p>
                Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                Impedit, ea iste molestias similique quo quia odio ab veniam
                minus cum labore deleniti. Aspernatur ipsum corrupti maiores
                temporibus cumque impedit doloribus.
              </p>
            </div>
            <div className="user">
              <p>About The Seller</p>
              <div className="seller-info">
                <img
                  src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                  alt=""
                />
                <div className="text">
                  <span className="seller-name">Ali Haseni</span>
                  <span className="stars">
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                  </span>
                  <button>Contact The seller</button>
                </div>
              </div>
              <div className="user-informations">
                <div className="top">
                  <div className="col">
                    <div className="inf">
                      <span>From</span>
                      <p>SYRIA</p>
                    </div>{" "}
                    <div className="inf">
                      <span>Avg.Finishing Time</span>
                      <p>10 hours</p>
                    </div>{" "}
                    <div className="inf">
                      <span>Language</span>
                      <p>Arabic</p>
                    </div>
                  </div>
                  <div className="col">
                    <div className="inf">
                      <span>Working Since</span>
                      <p>2021</p>
                    </div>{" "}
                    <div className="inf">
                      <span>Last Work</span>
                      <p>Web Store</p>
                    </div>
                  </div>
                </div>
                <hr />
                <div className="bottom">
                  <i>
                    From Syria Aleppo Living in Turkiye Since 2012, Trying to
                    Make Sites And Have Fun While Making them, Hope My Work Get
                    Your Intrest.
                  </i>
                </div>
              </div>

              <div className="priviews">
                <p className="my-8 text-[24px]">Reviews</p>
                <div className="review mb-7">
                  <div className="profile flex gap-2">
                    {" "}
                    <img
                      src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                      alt=""
                    />
                    <div className="text">
                      <p>Ali Haseni</p>
                      <div className="from">
                        <span>
                          <FontAwesomeIcon icon={faEarthAmericas} />
                        </span>{" "}
                        <span className=" text-gray-600 font-bold">
                          United States
                        </span>
                      </div>
                    </div>
                  </div>

                  <div className="stars my-4">
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                  </div>

                  <p className=" text-gray-600">
                    Lorem ipsum, dolor sit amet consectetur adipisicing elit.
                    Veniam quam laboriosam, earum, maiores quidem ad, quo sed
                    optio expedita quis voluptates libero
                  </p>

                  <div className="likes mb-6 flex gap-3 mt-5">
                    <span className=" font-bold">Helpful ?</span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsUp} /> yes
                    </span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsDown} /> No
                    </span>
                  </div>

                  <hr />
                </div>{" "}
                <div className="review mb-7">
                  <div className="profile flex gap-2">
                    {" "}
                    <img
                      src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                      alt=""
                    />
                    <div className="text">
                      <p>Ali Haseni</p>
                      <div className="from">
                        <span>
                          <FontAwesomeIcon icon={faEarthAmericas} />
                        </span>{" "}
                        <span className=" text-gray-600 font-bold">
                          United States
                        </span>
                      </div>
                    </div>
                  </div>

                  <div className="stars my-4">
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                  </div>

                  <p className=" text-gray-600">
                    Lorem ipsum, dolor sit amet consectetur adipisicing elit.
                    Veniam quam laboriosam, earum, maiores quidem ad, quo sed
                    optio expedita quis voluptates libero
                  </p>

                  <div className="likes mb-6 flex gap-3 mt-5">
                    <span className=" font-bold">Helpful ?</span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsUp} /> yes
                    </span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsDown} /> No
                    </span>
                  </div>

                  <hr />
                </div>{" "}
                <div className="review mb-7">
                  <div className="profile flex gap-2">
                    {" "}
                    <img
                      src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                      alt=""
                    />
                    <div className="text">
                      <p>Ali Haseni</p>
                      <div className="from">
                        <span>
                          <FontAwesomeIcon icon={faEarthAmericas} />
                        </span>{" "}
                        <span className=" text-gray-600 font-bold">
                          United States
                        </span>
                      </div>
                    </div>
                  </div>

                  <div className="stars my-4">
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                  </div>

                  <p className=" text-gray-600">
                    Lorem ipsum, dolor sit amet consectetur adipisicing elit.
                    Veniam quam laboriosam, earum, maiores quidem ad, quo sed
                    optio expedita quis voluptates libero
                  </p>

                  <div className="likes mb-6 flex gap-3 mt-5">
                    <span className=" font-bold">Helpful ?</span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsUp} /> yes
                    </span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsDown} /> No
                    </span>
                  </div>

                  <hr />
                </div>{" "}
                <div className="review mb-7">
                  <div className="profile flex gap-2">
                    {" "}
                    <img
                      src="https://fiverr-res.cloudinary.com/images/t_main1,q_auto,f_auto,q_auto,f_auto/gigs3/105848244/original/6e8130e0ad1ac457747e10617151cae561e4fe20/create-a-premium-and-modern-wordpress-website.jpg"
                      alt=""
                    />
                    <div className="text">
                      <p>Ali Haseni</p>
                      <div className="from">
                        <span>
                          <FontAwesomeIcon icon={faEarthAmericas} />
                        </span>{" "}
                        <span className=" text-gray-600 font-bold">
                          United States
                        </span>
                      </div>
                    </div>
                  </div>

                  <div className="stars my-4">
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                    <FontAwesomeIcon icon={faStar} />
                  </div>

                  <p className=" text-gray-600">
                    Lorem ipsum, dolor sit amet consectetur adipisicing elit.
                    Veniam quam laboriosam, earum, maiores quidem ad, quo sed
                    optio expedita quis voluptates libero
                  </p>

                  <div className="likes mb-6 flex gap-3 mt-5">
                    <span className=" font-bold">Helpful ?</span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsUp} /> yes
                    </span>
                    <span className=" cursor-pointer">
                      <FontAwesomeIcon icon={faThumbsDown} /> No
                    </span>
                  </div>

                  <hr />
                </div>
              </div>
            </div>
          </div>

          {/* Start Right SECTION */}

          <div className="right">
            <div className="head">
              <span className="title">Clothes Web Store</span>{" "}
              <span className="price">$ 119.99</span>
            </div>
            <div className="text">
              <p>
                An online store may also be called an e-web-store, e-shop,
                e-store, Internet shop, web-shop.
              </p>
            </div>
            <div className="duration">
              <div className="duration-left">
                <FontAwesomeIcon icon={faClock} /> 2 Days Delivery
              </div>
              <div className="duration-right">
                <FontAwesomeIcon icon={faSpinner} /> 3 Revisions
              </div>
            </div>
            <div className="features">
              <div>
                <FontAwesomeIcon icon={faCheck} />{" "}
                <span>Responsive Website</span>
              </div>{" "}
              <div>
                <FontAwesomeIcon icon={faCheck} />{" "}
                <span>Responsive Website</span>
              </div>{" "}
              <div>
                <FontAwesomeIcon icon={faCheck} />{" "}
                <span>Responsive Website</span>
              </div>{" "}
              <div>
                <FontAwesomeIcon icon={faCheck} />{" "}
                <span>Responsive Website</span>
              </div>
            </div>

            <button>Continue</button>
          </div>
        </div>
      </div>
    </>
  );
}

export default Gig;
